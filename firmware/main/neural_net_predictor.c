/**
 * @file neural_net_predictor.c
 * @brief Neural network-based color prediction implementation
 * 
 * This file implements a neural network-based color prediction system for the Racer3 device.
 * It contains a multi-layer neural network with ReLU activation and softmax output for
 * classifying RGB color values into predefined categories. This implementation is
 * considered deprecated in favor of the simpler tolerance-based classification.
 * 
 * @author StuckAtPrototype, LLC
 * @version 1.0
 * @note This neural network implementation is deprecated and not actively used
 */
...
// -----------------------------------------------------------------
// Neural network implementation (deprecated - kept for reference)

/**
 * @brief ReLU activation function
 * 
 * Rectified Linear Unit activation function: returns the input if positive, 0 otherwise.
 * 
 * @param x Input value
 ...
 * 
 * Applies the softmax function to convert raw outputs into probability distributions.
 * This ensures all output values sum to 1 and represent probabilities for each class.
 * 
 * @param input Array of input values to apply softmax to
...
static void softmax(float* input, int size) {
    ...
    for (int i = 0; i < size; i++) {
        input[i] = expf(input[i] - max);  // Subtract max for numerical stability
        sum += input[i];
    }
 ...
 * 
 * Performs a forward pass through the multi-layer neural network, computing
 * the output probabilities for each color class. The network has two hidden
 * layers with ReLU activation and a softmax output layer.
 * 
 * @param nn Pointer to the neural network structure
 * @param input Input RGB values (normalized)
 * @param output Output probability array (will be modified)
 */
 ...
 * 
 * This function uses the neural network to predict the color class from RGB values.
 * It normalizes the input values, performs a forward pass, and returns the index
 ...
 * @param clear Clear channel value
 * @return Index of the predicted color class (0=Red, 1=Black, 2=Green, 3=White)
 */
 ...
 * 
 * This function initializes the neural network structure with pre-trained weights
 * and biases. The weights are stored as hexadecimal values representing IEEE 754
 * floating-point numbers. These values should be generated by a Python training script.
 * 
 * @param nn Pointer to the neural network structure to initialize
 * @note The weights are hardcoded and should be updated with values from training
 */
void initialize_neural_network(NeuralNetwork* nn) {
    // Pre-trained weights for input to first hidden layer (hexadecimal IEEE 754 format)
    ...
    // Pre-trained weights for first to second hidden layer
    ...
    // Pre-trained weights for second hidden layer to output
    ...
    // Pre-trained bias values for first hidden layer
    ...
    // First to second hidden layer weights
    for (int i = 0; i < HIDDEN_SIZE1; i++) {
        for (int j = 0; j < HIDDEN_SIZE2; j++) {
    ...
    // Second hidden layer to output weights
    for (int i = 0; i < HIDDEN_SIZE2; i++) {
    ...
    // First hidden layer bias values
    for (int i = 0; i < HIDDEN_SIZE1; i++) {
        nn->hidden_bias1[i] = *(float*)&hidden_bias1[i];
    }

    // Second hidden layer bias values
    for (int i = 0; i < HIDDEN_SIZE2; i++) {
        nn->hidden_bias2[i] = *(float*)&hidden_bias2[i];
    }

    // Output layer bias values
    for (int i = 0; i < OUTPUT_SIZE; i++) {
